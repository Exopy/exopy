...
This section will focus on the functionalities offered by the plugins
...
One usual need in plugin applications is to make available to all other parts of
the application some function (for example the possibility to request the use
of a driver) or to let other parts of the application know what is the state of a
plugin (for example a list of all the available drivers).

One could of course directly access the plugin to get this kind of information but
in a plugin application it is a good to avoid such interferences. This
information management is actually delegated to two specific plugins :
...
- the 'enaml.workbench.core' plugin is in charge of managing commands which are
  ...
  (this is the only case in which one needs to directly access a plugin).
  ...
  contributing it.
- the 'ecpy.app.states' plugin is in charge of managing states and as such it allows 
  access to a read-only representation of some of the attributes of a
  ...
  constituting the state. If you need to assess a state you should
...
- an id which must be unique (it is a dot separated name).
- a handler which is a function taking as argument an |ExecutionEvent| instance.
  The execution event allows access to the application workbench
...
(for example discovering extension packages, or adding new logger handlers) or
some special clean up operations when the application closes. It may also need
to have a say so about whether or not the application can close (if a measure
is running the application should not close without a huge warning). The
...
.. note::
   ...
   The customization of the start up and closing of the application should only be
...
If any of your plugin needs to retain user preferences from one application run
...
requests it. Finally, a |Preferences| object to the
...
During the application lifetime errors can occur and the user needs to be
...
reporting the error. The selected handler determines the expected keywords.
...
execution of some code is completed. To this effect the error plugin provides
...
.. note::
    As using this mechanism will cause a window to be displayed for the users'
    sake these commands should be called only from functions/methods directly
...
|RuntimeDependencyAnalyser|, which allows the use of the same kind of dependency in
objects with totally different structures and for which the same scheme of
