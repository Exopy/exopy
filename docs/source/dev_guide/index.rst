...
Exopy is designed with the idea that user should be able to extend it to fit
...
.. note::
   ...
   When writing code for Exopy, or an extension packages you should follow the
   ...
   order for a contribution to Exopy or one of its official extension package
   to be accepted it must follow those style guides.

.. note::

    Exopy does not export any name in the \_\_init\_\_.py module. However for 
    ease of use the objects necessary to extend Exopy functionality are exported
    in the api.py file associated with each plugin.
   
.. note::

    Exopy is built on top of Atom and Enaml. Please have a look at
