...
def create_dir(path, package_name, opts):
    ...
    if not os.path.isdir(path):
        ...
        if opts.dryrun:
            ...
            print('Creating folder for %s package' % package_name)
            os.makedir(path)


def write_file(path, name, text, opts):
    """Write the output file for module/package <name>."""
    fname = path.join(path, '%s.%s' % (name, opts.suffix))
    if opts.dryrun:
...
def create_package_file(package, module, opts):
    """
    """
    pass


def recurse_tree(path, package_name, outputdir, excludes, opts):
    """Look for every file in the directory tree and create the corresponding
    ReST files.
    """
    followlinks = getattr(opts, 'followlinks', False)
    includeprivate = getattr(opts, 'includeprivate', False)
    if hasattr(opts, 'suffixes'):
        suffixes = SUFFIXES.union(opts.suffixes.split(' '))
    else:
        suffixes = SUFFIXES

    for root, subs, files in walk(path, followlinks=followlinks):
        # document all specified module files (that aren't excluded)
        files = sorted(f for f in files
                       if path.splitext(f)[1] in suffixes and
                       not is_excluded(path.join(root, f), excludes))
        is_pkg = INITPY in files
        if is_pkg:
            files.remove(INITPY)
        else:
            continue
        ...
        if is_pkg:
            ...
            for sub in subs:
                out = os.path.join(outputdir, sub)
                subname = makename(package_name, sub)
                create_dir(out, subname, opts)
                recurse_tree(os.path.join(path, sub), subname,
                             out, excludes, opts)

            for file_to_doc in files:
                if not shall_skip(path.join(path, file_to_doc), opts):
                    module = path.splitext(file_to_doc)[0]
                    create_module_file(package_name, module, opts)

            create_package_file(package_name, outputdir, subs, files, opts)
...
Note: By default this script will not overwrite already created files.""")
    ...
    parser.add_option('--suffixes', action='store', dest='suffixes',
                      help='Additional file suffixes to document.')
...
#
#
#def create_package_dir(root, master_package, subroot, files, opts, subs):
#    """Build the text of the file and write the file."""
...
#        text += '\n'
#
#    submods = [path.splitext(sub)[0] for sub in files
...
#        if is_pkg:
#            # we are in a package with something to document
#            if (subs or len(files) > 1 or
#                    not shall_skip(path.join(root, INITPY), opts)):
#                subpackage = root[len(rootpath):].lstrip(path.sep).\
#                    replace(path.sep, '.')
#                create_package_dir(root, root_package, subpackage,
#                                   files, opts, subs)
...
#            for file_to_doc in files:
#                if not shall_skip(path.join(rootpath, file_to_doc), opts):
#                    module = path.splitext(file_to_doc)[0]
